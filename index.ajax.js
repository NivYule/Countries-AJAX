...
async function init () {
    ...
    function getCardItem (countryData) {
        ...
        const card = $("<div></div>").addClass("card").css({"width": "23rem", "height": "15rem", "margin": "5px", "display": "inline-flex"});
        const cardImg = $("<img>").addClass('card-img-top').attr("src", flag).css({"width": "50px", "border": "1px solid black"});
        const cardHead = $("<div></div>").addClass('card-body');
        const cardBody = $("<div></div>").addClass('card-text').css("text-align", "center");
        const cardTitle = $("<h6></h6>").addClass('card-title').css({"margin-left": "15px", "display": "inline-flex"}).text(`${name}`);
        const cardInfo = $("<p></p>").html(`<b>Population:</b> ${population}<br><b>Capital:</b> ${capital}<br><b>Region:</b> ${region}`);
